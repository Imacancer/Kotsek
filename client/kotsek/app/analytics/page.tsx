...
const AnalyticsDashboard = () => {
  ...
  const selected = Object.entries(reportSections)
    .filter(([_, val]) => val)
    .map(([key]) => key)
    .join(",");

  const handleGenerateReport = async (sections: typeof reportSections) => {
    ...
    try {
      ...
      const apiUrl = `${SERVER_URL}/analytics/weekly-summary?start_date=${encodeURIComponent(
        ...
        endDateStr
      )}&sections=${encodeURIComponent(selected)}&user=${encodeURIComponent(
        username
      )}`;
          ...
          <div className="flex items-center gap-4">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="w-56">
                {[
                  { key: "summary", label: "Traffic Overview" },
                  { key: "daily", label: "Daily Traffic Breakdown" },
                  { key: "hourly", label: "Hourly Traffic Patterns" },
                  { key: "utilization", label: "Parking Slot Utilization" },
                  { key: "customers", label: "Customer Analytics" },
                  { key: "duration", label: "Parking Duration Analysis" },
                ].map(({ key, label }) => (
                  <DropdownMenuCheckboxItem
                    key={key}
            ...
            <Button
              onClick={() => handleGenerateReport(reportSections)}
